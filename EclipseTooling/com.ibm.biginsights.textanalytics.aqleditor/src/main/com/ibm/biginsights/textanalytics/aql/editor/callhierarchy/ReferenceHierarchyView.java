 ...
 *
 */
public class ReferenceHierarchyView extends ViewPart
{


 
	public static final String ID = "com.ibm.biginsights.textanalytics.aql.editor.callhierarchy.ReferenceHierarchyView.view"; //$NON-NLS-1$
  ...
  protected static String currToken = null; //This value is treated as the aql view which would be the root for the reference hierarchy tree.
  ...
  public void createPartControl(Composite parent) {
    PlatformUI.getWorkbench().getHelpSystem().setHelp(parent, "com.ibm.biginsights.textanalytics.tooling.help.aql_reference_hierarchy");
    this.setContentDescription (Messages.ReferenceHierarchy_DESC);
    getRoot();
    ...
    viewer.getTree ().getAccessible ().addAccessibleListener (new AccessibleAdapter() {
      ...
      public void getName (AccessibleEvent e)
      {
        if (e.childID == ACC.CHILDID_SELF) { //If focus is on the tree and no elements have been selected.
          e.result = getPartName () + " : " + Messages.ReferenceHierarchy_DESC; //$NON-NLS-1$
        }
      }
    });
  ...
  protected HierarchyRootNode getRoot() {
    ...
    root.setIfDependencyHierarchyRoot (false);
    return root;
  }
