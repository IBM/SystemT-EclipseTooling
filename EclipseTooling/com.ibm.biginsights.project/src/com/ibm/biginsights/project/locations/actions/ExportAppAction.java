...
public class ExportAppAction extends Action {

	public ExportAppAction() {
	...
	public void run() {
		...
		try {					
			...
			ParameterizedCommand pcmd = commandService.deserialize("org.eclipse.ui.file.export("+ //$NON-NLS-1$
					"exportWizardId=com.ibm.biginsights.application.publishwizard"+ //$NON-NLS-1$
					")"); //$NON-NLS-1$
			handlerService.executeCommand(pcmd, null);
		} catch (Exception e) {			
			IStatus status = new Status(IStatus.ERROR, Activator.PLUGIN_ID, e.getMessage());
			Activator.getDefault().getLog().log(status);
			ErrorDialog.openError(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), Messages.EXPORTAPPACTION_TITLE, Messages.EXPORTAPPACTION_MESSAGE, status);
		}
