...
public class ShowPrevDocWithAnnotHandler extends AbstractTreeViewHandler {
		...
		if (metaModel != null) {
			...
			if (part instanceof AQLResultTreeView) {
				...
				if (thisDoc.contains(Constants.ALL_DOCS)) {
					...
					result = metaModel.getNextPrevious(thisDoc, false, al,
							docSchema);
				}
			}
	...
	public void refresh() {
		...
		if (commandService != null) {
			commandService
					.refreshElements(
							"com.ibm.biginsights.textanalytics.treeview.commands.ShowPreviousWithAnnot",
							null);//$NON-NLS-1$
		}
	}
}
