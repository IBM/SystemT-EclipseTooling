...
public class ChooseProjectDialog extends Dialog
{
  ...
  private final String title = Messages.choose_project_dialog_title;

  protected Button okButton;
  private Combo projects_combo;
  ...
  protected void buttonPressed (int buttonId)
  {
    if (buttonId == IDialogConstants.OK_ID) {
      projectName = projects_combo.getText ();
    }
    else {
      projectName = null;
    }
  ...
  protected Control createDialogArea (Composite parent)
  {
    ...
    type_label.setText (Messages.choose_project_dialog_project);

    projects_combo = new Combo (data_section, SWT.READ_ONLY | SWT.BORDER);
    projects_combo.setLayoutData (new GridData (SWT.FILL, SWT.CENTER, true, false, 1, 1));
  ...
  {
    projects_combo.removeAll ();

    int i = 0;
    int selection_index = i;

    String current = ActionPlanView.projectName;

    for (IProject project : AqlProjectUtils.getWorkspaceRoot ().getProjects ()) {
      if (!project.isOpen ())
        continue;

      String name = project.getName ();
      projects_combo.add (name);
      if (name.equals (current)) selection_index = i;
      i++;
    }

    projects_combo.select (selection_index);
  }
