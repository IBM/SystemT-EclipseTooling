...
public class LocationJAQLShellAction extends Action {
	private IWorkbenchPage page;
	private ISelectionProvider selectionProvider;

	public LocationJAQLShellAction(IWorkbenchPage page, ISelectionProvider selectionProvider) {
		setText(Messages.LOCATIONJAQLSHELLACTION_TITLE);
	...
	public boolean isEnabled() {
	    ...
	    {
	    	// if element is IBigInisightsLocation, then enable jaqlShell unless the location is on GPFS or the GPFS cluster is mounted
	...
	public void run() {		
		...
		if (structuredSelection.getFirstElement() instanceof IBigInsightsLocation) {
			...
			try {					
				...
				ParameterizedCommand pcmd = commandService.deserialize("com.ibm.biginsights.jaql.launch.command("+ //$NON-NLS-1$
						BIConstants.LOCATION_PARM+"="+ //$NON-NLS-1$
