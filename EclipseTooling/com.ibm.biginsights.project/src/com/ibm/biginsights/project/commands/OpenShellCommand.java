...
public class OpenShellCommand extends AbstractHandler {

	@Override
	public Object execute(ExecutionEvent event) throws ExecutionException {
		List<ArtifactType>artifacts = new ArrayList<ArtifactType>();
		ArtifactType _selectedArtifactType = null;
		IBigInsightsLocation _selectedBIServer = null;
		
		// read the contributors to the extension point and create a dialog for selection
    	IConfigurationElement[] config = Platform.getExtensionRegistry().getConfigurationElementsFor("com.ibm.biginsights.artifacts.shells"); //$NON-NLS-1$
		for (IConfigurationElement element:config)
		{			
			String name = element.getAttribute("name"); //$NON-NLS-1$
			String desc = element.getAttribute("desc"); //$NON-NLS-1$
			String shellCommandId = element.getAttribute("shellCommandId"); //$NON-NLS-1$
			String helpUrl = element.getAttribute("helpUrl"); //$NON-NLS-1$			
			String minBigInsightsVersion = element.getAttribute("minBigInsightsVersion"); //$NON-NLS-1$
			ArtifactType artifactType = new ArtifactType(name, desc, shellCommandId, helpUrl, minBigInsightsVersion);
			artifacts.add(artifactType);
		}

		// show dialog with all possible create program types
		ShellSelectionDialog dlg = new ShellSelectionDialog(Activator.getActiveWorkbenchShell(), null, artifacts);		
		
		if (dlg.open()==Window.OK) {
			_selectedBIServer = (IBigInsightsLocation)dlg.getResult()[0];
			_selectedArtifactType = (ArtifactType)dlg.getResult()[1];			
		}

		if (_selectedArtifactType!=null && _selectedBIServer!=null) {
...
//				PlatformUI.getWorkbench().getHelpSystem().displayHelpResource(helpUrl);
			...
			try {
				...
				final String serverName = _selectedBIServer.getLocationName();
				Display.getDefault().asyncExec(new Runnable() {								
					public void run() {
						try {
							...
							Command command = commandService.getCommand(actionId);
							IParameter serverIdParm = command.getParameter(BIConstants.LOCATION_PARM);
							Parameterization parmServerId = new Parameterization(serverIdParm, serverName); 			
							final ParameterizedCommand parmCommand = new ParameterizedCommand(command, new Parameterization[] { parmServerId });
